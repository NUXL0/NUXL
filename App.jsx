  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="absolute inset-0">
        <div className="absolute inset-0 bg-gradient-to-br from-green-950/30 via-black to-green-950/10" />
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-5 border-b border-green-500/20 bg-black/40 backdrop-blur-md">
        ...
        <nav className="hidden md:flex gap-8 text-sm text-gray-300">
          ...
          <button onClick={() => setLocation("/admin")} className="hover:text-green-400 transition-colors duration-200">
            Admin Panel
          </button>
        </nav>
        ...
        <button
          ...
          className="px-5 py-2 rounded-xl bg-green-500 hover:bg-green-400 active:bg-green-600 transition-all duration-200 text-black font-semibold shadow-lg shadow-green-500/30"
        >
          Login
        </button>
      </header>
        ...
        <div className="max-w-4xl">
          <div className="flex justify-center mb-6">
            <img
              src={logo}
              alt="NUXL Logo"
              className="w-36 h-36 md:w-48 md:h-48 object-contain drop-shadow-[0_0_40px_rgba(74,222,128,0.5)]"
            />
          </div>
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-wide leading-tight">
            Welcome to{" "}
            <span className="text-green-400 drop-shadow-[0_0_30px_rgba(74,222,128,0.4)]">NUXL</span>
          </h2>

          <p className="mt-6 text-gray-300 text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
            Professional Cyber Styled Website with Modern Dashboard,
            Full Admin Access, Security Analytics &amp; Powerful UI Design.
          </p>

          <div className="mt-10 flex flex-wrap items-center justify-center gap-5">
            <button
              onClick={() => setLocation("/login")}
              className="px-8 py-4 rounded-2xl bg-green-500 hover:bg-green-400 active:scale-95 text-black font-bold transition-all duration-200 shadow-2xl shadow-green-500/40"
            >
              Open Dashboard
            </button>
            <button className="px-8 py-4 rounded-2xl border border-green-500/60 text-green-400 hover:bg-green-500/10 hover:border-green-400 active:scale-95 transition-all duration-200">
              View Features
            </button>
          </div>
        </div>
      ...
      <section className="relative z-10 px-6 md:px-16 py-12 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-white/5 border border-green-500/20 rounded-3xl p-6 backdrop-blur-xl hover:border-green-400/60 hover:scale-105 transition-all duration-300 shadow-lg shadow-green-500/5 group cursor-default"
          >
            <h3 className="text-gray-400 text-xs uppercase tracking-widest font-medium">{card.title}</h3>
            <div className={`mt-4 text-4xl font-black ${card.accent} drop-shadow-[0_0_15px_currentColor]`}>
              {card.value}
            </div>
      ...
      <section className="relative z-10 px-6 md:px-16 py-20">
        <div className="bg-white/5 border border-green-500/20 rounded-3xl p-8 md:p-12 backdrop-blur-xl shadow-2xl shadow-green-500/10">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-12">
            <div>
              <div className="text-xs font-semibold tracking-widest uppercase text-green-500/70 mb-3">
                Control Center
              </div>
              <h2 className="text-3xl md:text-4xl font-black text-white">
                Professional <span className="text-green-400">Admin Panel</span>
              </h2>
              <p className="mt-3 text-gray-400 max-w-2xl text-sm md:text-base leading-relaxed">
                Manage users, monitor activity, control permissions,
                analytics, logs, and security systems from one powerful dashboard.
              </p>
            </div>
            <button
              onClick={() => setLocation("/login")}
              className="shrink-0 px-8 py-4 rounded-2xl bg-green-500 hover:bg-green-400 active:scale-95 text-black font-bold transition-all duration-200 shadow-lg shadow-green-500/30"
            >
              Access Admin
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className="bg-black/40 border border-green-500/20 rounded-2xl p-6 hover:border-green-500/40 transition-colors duration-300 group"
              >
                ...
                <ul className="mt-4 space-y-3 text-gray-400 text-sm">
                  {panel.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-2 hover:text-gray-300 transition-colors">
      ...
      <footer className="relative z-10 border-t border-green-500/20 py-8 text-center text-gray-500 text-sm bg-black/40 backdrop-blur-md">
        <span className="text-green-500/50">©</span> 2026 NUXL — Cyber Dashboard &amp; Admin Panel.
      </footer>
